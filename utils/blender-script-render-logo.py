...
def create_text():
    ...
    font_path = os.path.join(os.path.expanduser("~"), "Library/Fonts", "Comic Code Bold.otf")
...
def assign_text_material(text_obj):
    ...
    principled_text.inputs['Metallic'].default_value = 0.8
    principled_text.inputs['Roughness'].default_value = 0.1
    if "Clearcoat" in principled_text.inputs:
        principled_text.inputs["Clearcoat"].default_value = 0.5
    ...
    mix_shader.inputs[1].default_value = principled_text.inputs['Base Color'].default_value
